...
| Phase 4: Harness 正规化收口 | open | 把 2026-09-03 架构审计中“spec 态/空壳态”的 harness 构件落到代码态：评估自动采集、上下文装配/断点恢复、Reflect 机械化、能力层 MCP 化、agent→telemetry 正名 | hb-session | TASK-092..098 | ① harness 8 项检查表复评全 code 态（或明确标注“由宿主 CLI 承担”）；② evaluation/metrics 有自动采集的真实数据；③ 复评 ADR 入 `knowledge/architecture/` |
| Phase 5: 点子→成品流水线 | open | 跨项目 DAG + 阶段人工闸门 + 成品验收（原 Phase 4，2026-09-03 编号顺延，为 Phase 4 后继） | hb-session | —（前置 Phase 3 ✅、Phase 4） | 愿景，需逐步验证 |

## 变更记录

- 2026-09-03：新增 **Phase 4: Harness 正规化收口**（open），原 Phase 4（点子→成品流水线）编号顺延为 Phase 5（历史条目中“Phase 4”字样不回改，均指今日之 Phase 5）。提议来源：AIOS 架构 harness 正规度审计（对照 8 项 harness 构件清单，识别出评估自动采集/上下文装配/Reflect 机械化/MCP 化/telemetry 正名五处 spec 态缺口；明确不做：cognition 机械化、自研内层 agent loop、A2A、RAG 索引）；人工确认：hb（本会话指令）；Manager 落盘。对应 TASK-092..098 已创建（含 frontmatter 依赖链与验收标准）。
