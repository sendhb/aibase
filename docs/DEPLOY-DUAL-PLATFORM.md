...
>
> 依据（唯一真相）：
> - `aimonitor/docs/MONITOR-SPEC.md` §3.1（Agent 推送模式契约）
> - `aimonitor/docs/OPERATIONS.md`（运维手册）
> - `aibase/kit/tools/agent/README.md`（agent 组件说明）
> - 本手册与实现行为逐项核对（2026-08-18）。
...
# 心跳/事件由 autoloop 产生（autoloop 是 bash 脚本，Windows 上需 Git Bash / WSL 运行；
...
autoloop 可在 Git Bash/WSL 下运行以产生心跳与事件流。
...
# ── 项目侧（Linux / Git Bash）──
bash kit/cli/mkproject ~/code/backend-api --profile backend   # 创建项目（Python 跨平台）
